...
def extract_article_body(html: str) -> Dict[str, str]:
    ...
    try:
        ...
        for p in p_tags:
            ...
            filter_keywords = [
                # 著作権・プライバシー
                '著作権', 'Copyright', 'プライバシー', 'NIKKEI', 'No reproduction',
                # 有料記事関連
                '有料記事', '【有料記事】', '有料登録', '無料登録', 'すべての記事が読み放題',
                '会員限定', '会員限定です', '登録すると続きを', 'この記事は会員限定',
                # Prime・サブスクリプション関連
                'NIKKEI Prime', 'Prime について', '朝夕刊や電子版では',
                'プレミアム', 'プレミアムプラン',
                # メール配信・ニュースレター
                'メールアドレス', 'メール配信', 'ニュースレター', 'フォロー',
                '編集者が選んだ', 'ニュース５本', 'お届けします', '週５回配信',
                # その他テンプレートテキスト
                '記事消費', '会議資料', 'メッセージ', '※掲載される投稿',
                'ご入力内容', 'エキスパートへ', 'Think!事務局',
                # サービス紹介
                '今後も様々な切り口で', '切り口でサービスを開始',
                '企業での記事共有', '転載・複製', '注文印刷'
            ]
